...
struct Agent {
    pair<size_t, size_t> parkingPoint, currentPoint;
...
//    size_t deliveryY = 3;
    ...
    string filename =
            dataPath + "/task/" + mapName + "-" + to_string(agentNum) + "-" + to_string(k) + "-" + to_string(seed);
    if (release) {
        filename += "-release";
    }
    filename += ".task";
...
//                cout << i << " " << j << endl;
    ...
    vector<Agent> agents(agentNum);
    for (size_t i = 0; i < agentNum; i++) {
        auto agentPoint = parkingPoints.back();
        parkingPoints.pop_back();
        agentConfigs << agentPoint.first << " " << agentPoint.second << endl;
        agents[i].parkingPoint = agents[i].currentPoint = agentPoint;
    }

    for (size_t n = 0; n < agentNum * k; n++) {
        auto startPoint = taskPoints[sampleTask(g)];
        auto endPoint = taskPoints[sampleTask(g)];
        while (startPoint == endPoint) {
            endPoint = taskPoints[sampleTask(g)];
        }
        size_t selectedAgent = agentNum;
        size_t minTime = numeric_limits<size_t>::max() / 2;
        for (size_t i = 0; i < agentNum; i++) {
            auto scenario = Scenario(i, map, {agents[i].currentPoint, startPoint, endPoint}, 0, 0);
            solver.initScenario(&scenario);
            auto distance = calculateDistance(solver);
            if (agents[i].lastTimestamp + distance < minTime) {
                selectedAgent = i;
                minTime = agents[i].lastTimestamp + distance;
            }
        }
        assert(selectedAgent < agentNum);
        agents[selectedAgent].currentPoint = endPoint;
        agents[selectedAgent].lastTimestamp = minTime;
        int startTime = 0;
        taskConfigs << startPoint.first << " " << startPoint.second << " "
                    << endPoint.first << " " << endPoint.second << " " << minTime << " " << startTime << endl;
    }
